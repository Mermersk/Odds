...
public class Scraper {
	...
	public Scraper(String deild, String lid) {
		league = deild;
		team = lid;
	}
	...
	public void setTeam(String lid) {
		team = lid;
	}
	...
	public String connectTeam() {
		...
		for (int i = 0; i < one_team_home.size(); i++) {
			if (one_team_home.get(i).text().contains(team)) {
				team_number_home = i;
			}
			if (one_team_away.get(i).text().contains(team)) {
				team_number_away = i;
			}
		}
	...
	public Element connectAllResults() throws IOException {
	...
	public ArrayList<String> getHomePlayedMatches() {
		...
		for (int i = 2; i < ar_table_elements.size(); i++) {
			...
			if (ar_table_elements.get(i).text().contains(team) == true) {
	...
	public ArrayList<String> getAwayOrderlyTable() {
		...
		List<List> list_of_mixed_types = new ArrayList<List>();
		
		ArrayList<String> list_of_strings = new ArrayList<String>();
		ArrayList<Integer> list_of_integers = new ArrayList<Integer>();
		...
		//list_of_mixed_types.add(list_of_strings);
		//list_of_mixed_types.add(list_of_integers);
	}
...
public ArrayList<String> getHomeOrderlyTable() {
		...
		System.out.println(orderly_home_table.get(3));
		System.out.println(orderly_home_table.get(3).length());
	...
	public int calculateHomeStrength() {
		...
		Pattern pattern = Pattern.compile(team); //Regex
		...
		String opponent = "";
		// - [(\p{L}+) (\p{L}+)]+
		Pattern pattern2 = Pattern.compile(team + " - ((\\w+) (\\w+))", Pattern.UNICODE_CASE);
	...
	public int calculateAwayStrength(String away_team) {
